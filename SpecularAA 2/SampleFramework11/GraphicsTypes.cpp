...
void RWBuffer::Initialize(ID3D11Device* device, DXGI_FORMAT format, uint32 stride, uint32 numElements, bool32 rawBuffer)
{
    Format = format;
...
void StructuredBuffer::Initialize(ID3D11Device* device, uint32 stride, uint32 numElements, bool32 useAsUAV,
                                    bool32 appendConsume, bool32 useAsDrawIndirect, const void* initData)
{
    Size = stride * numElements;
    Stride = stride;
    NumElements = numElements;
...
void StructuredBuffer::ReadFromFile(const wchar* path, ID3D11Device* device)
{
    ...
    bool32 useAsUAV, useAsDrawIndirect, appendConsume;
    DWORD bytesRead = 0;

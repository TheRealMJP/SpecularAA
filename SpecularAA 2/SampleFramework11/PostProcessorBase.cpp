...
void PostProcessorBase::Initialize(ID3D11Device* device)
{
    ...
    quadVS.Attach(CompileVSFromFile(device, L"SampleFramework11\\Shaders\\Quad.hlsl", "QuadVS", "vs_4_0", NULL, &byteCode));
...
void PostProcessorBase::PostProcess(ID3D11PixelShader* pixelShader, const wchar* name)
{
    ...
    context->OMSetRenderTargets(static_cast<uint32>(outputs.size() + uaViews.size()), rtViews, NULL);

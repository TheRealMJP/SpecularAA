...
void Skybox::Initialize(ID3D11Device* device)
{
    ...
    vertexShader.Attach(CompileVSFromFile(device, L"SampleFramework11\\Shaders\\Skybox.hlsl", "SkyboxVS", "vs_4_0", NULL, &byteCode));
    geometryShader.Attach(CompileGSFromFile(device, L"SampleFramework11\\Shaders\\Skybox.hlsl", "SkyboxGS", "gs_4_0"));
    emPixelShader.Attach(CompilePSFromFile(device, L"SampleFramework11\\Shaders\\Skybox.hlsl", "SkyboxPS", "ps_4_0"));
    skyPixelShader.Attach(CompilePSFromFile(device, L"SampleFramework11\\Shaders\\Skybox.hlsl", "SkyModelPS", "ps_4_0"));
    ...
    DXCall(device->CreateBuffer(&desc, NULL, &vsConstantBuffer));

    // Create the GS constant buffer
    desc.ByteWidth = CBSize(sizeof(GSConstants));
    DXCall(device->CreateBuffer(&desc, NULL, &gsConstantBuffer));

    // Create the PS constant buffer
    desc.ByteWidth = CBSize(sizeof(PSConstants));
    DXCall(device->CreateBuffer(&desc, NULL, &psConstantBuffer));
                        ...
                        uint32 rtIndex)
{
    ...
    RenderCommon(context, NULL, skyPixelShader, view, projection, bias, setStates, rtIndex);

...
{
    static ID3D11ComputeShaderPtr decodeTextureCS;
    static ID3D11ComputeShaderPtr decodeTextureArrayCS;

    static const uint32 TGSize = 16;

    if(decodeTextureCS.GetInterfacePtr() == NULL)
    {
        CompileOptions opts;
        opts.Add("TGSize_", TGSize);
        decodeTextureCS.Attach(CompileCSFromFile(device, L"SampleFramework11\\Shaders\\DecodeTextureCS.hlsl",
                                                 "DecodeTextureCS", "cs_5_0", opts.Defines()));

        decodeTextureArrayCS.Attach(CompileCSFromFile(device, L"SampleFramework11\\Shaders\\DecodeTextureCS.hlsl",
                                                      "DecodeTextureArrayCS", "cs_5_0", opts.Defines()));
    }
    ...
    DXCall(device->CreateTexture2D(&decodeTextureDesc, NULL, &decodeTexture));

    ID3D11UnorderedAccessViewPtr decodeTextureUAV;
    DXCall(device->CreateUnorderedAccessView(decodeTexture, NULL, &decodeTextureUAV));
